    ...
    context("When all parameters are valid", async function () {
      context("When the reward info is still within the limit", async function () {
        it("should still be able to update the new reward info", async function () {
          ...
          expect(noUpdateRewardInfo.endBlock).to.eq(this.farming.mockedBlock.add(11).toString());
          expect(await this.farming.rewardToken.balanceOf(await this.signers.admin.getAddress())).to.eq(balBefore);
          ...
          expect(updateRewardInfo.endBlock).to.eq(this.farming.mockedBlock.add(9).toString());
          expect(await this.farming.rewardToken.balanceOf(await this.signers.admin.getAddress())).to.eq(
            balBefore.add(
          ...
          await expect(
            this.farming.farmingRangeAsDeployer.updateRewardInfo(
              0,
              0,
              this.farming.mockedBlock.add(11).toString(), // back to initial
          ...
          expect(updateRewardInfoAgain.endBlock).to.eq(this.farming.mockedBlock.add(11).toString());
          expect(await this.farming.rewardToken.balanceOf(await this.signers.admin.getAddress())).to.eq(
            INITIAL_BONUS_REWARD_PER_BLOCK.mul(1) // added balance to rewards for range 1
              .sub(INITIAL_BONUS_REWARD_PER_BLOCK.add(2))
              .add(
      ...
      context("When we should add rewards to the pool within actual range", async function () {
        it("should have more rewards into pool but just recalculated from actual block", async function () {
          ...
          expect(updateRewardInfo.endBlock).to.eq(this.farming.mockedBlock.add(20).toString());
          ...
          expect(await this.farming.farmingRangeAsAlice.pendingReward(0, await this.signers.user.getAddress())).to.eq(
            INITIAL_BONUS_REWARD_PER_BLOCK.mul(updateBlock.sub(startBlock)).add(
    ...
    context("When some parameters are invalid", async function () {
      context("When the caller isn't the owner", async function () {
        it("should reverted since there is a modifier only owner validating the ownership", async function () {
          ...
          await expect(this.farming.farmingRangeAsAlice.setRewardInfoLimit(1)).to.be.revertedWith(
        ...
        it("should be reverted", async function () {
          ...
          await expect(
            this.farming.farmingRangeAsDeployer.updateRewardInfo(
              0,
              0,
              this.farming.mockedBlock.add(7).toString(),
              INITIAL_BONUS_REWARD_PER_BLOCK,
            ),
          ).to.reverted;
        });
